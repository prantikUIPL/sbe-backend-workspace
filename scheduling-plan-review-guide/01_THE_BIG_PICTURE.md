...
> **📘 THE PLAN** — everything above: the 3-layer shape, materialize-then-dispatch, stable identity, claim/reaper, DST-correct time, stop-conditions, and shipping *without waiting on* the SMS and DRR tracks, behind hard mechanical gates.
...
> **📦 WHERE IT LANDED — (combined-release docs, 2026-07-08)** — The fix-list below now has a home, and the shipping picture changed: Scheduling (76.6/77.8), DRR (77.9) and SMS (76.8) now ship **together as one combined release** — nothing is deferred anymore. Every S/X finding became a concrete delta in the [scheduling fixes addendum](../email_and_sms_docs/email_sms_combined_release_docs/EMAIL_SMS_SCHEDULING_FIXES_ADDENDUM.md) (the approved plan stays byte-for-byte untouched; where the two conflict, the addendum wins); the D-findings landed in the [DRR implementation plan](../email_and_sms_docs/email_sms_combined_release_docs/EMAIL_SMS_77.9_DRR_IMPLEMENTATION_PLAN.md); the [integration spine](../email_and_sms_docs/email_sms_combined_release_docs/EMAIL_SMS_COMBINED_RELEASE_INTEGRATION_SPINE.md) sequences all three tracks (milestones MS0–MS9); and the who-decides questions moved into the [open-questions register](../email_and_sms_docs/email_sms_combined_release_docs/EMAIL_SMS_COMBINED_RELEASE_OPEN_QUESTIONS.md) with owners and defaults. The plan's mailroom still ships without *waiting* on the other two tracks — its SKIP gates survive as the **interim boundary** until each track's un-gate milestone (the token-recipient gate lifts when the DRR engine deploys at MS6; the SMS gate via the H1 code flip at MS9 — deployed dark, with production sending behind the separate `sms_sending_enabled` launch toggle). The finding-by-finding map is in [file 8: The combined release](08_THE_COMBINED_RELEASE.md).
