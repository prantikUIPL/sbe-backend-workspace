...
The **read-only breakdown of one of the exhibitor's own orders** — everything the Order Details page needs in a single aggregate. Since **SBE-1147 (2026-07-26)** the body is **nested by show**: a `shows[]` list where each show carries its own **booth** lines (live `product_name` + snapshot `purchased_name`, size, price, per-booth `setup_fee`/`cleaning_fee`, subtotal), **add-ons**, **sponsorships**, and a single **onsite_contact** ({name,email,phone}|null). Alongside `shows[]`: a conditional **financial summary** (subtotal, coupon *or* gift-certificate, fees, savings, totals), settled **payment records**, the **agreement** acceptance block (with a presigned **`signature_url`** for the captured signature image), **additional_purchases[]** (each tagged with its `show:{id}`), a `can_download_invoice` flag, and a nullable top-level **`unassigned`** bucket for any line whose show can't be resolved (surfaced, never dropped). Every booth/sponsorship/add-on line also carries a **`id`**, an **`is_default_included`** flag (bundled/included vs purchased), and a **`parent: {id, type} | null`** ref so the FE can nest a bundled child under its parent line within the same show (**SBE-1147, 2026-07-27**). Sibling routes **list** an order's invoices and stream a **specific invoice PDF** by id. The endpoint is **product-gated** — a subscription/ppl_addon order returns **404**, same as a foreign or unknown id (no enumeration leak). Ownership is enforced from the JWT. Backs story **13.3**.
...
|---|---|---|---|
| `GET` | `/orders/:orderId` | The order-details aggregate, **nested by show** (`shows[]` → booths/add-ons/sponsorships/onsite_contact), plus financial summary, settled payments, agreement (+`signature_url`), `additional_purchases[]`, `unassigned`, `can_download_invoice`. Every line carries `id`/`is_default_included`/`parent:{id,type}`. **Product-gated** (subscription/ppl_addon → 404). | 13.3-a…ab |
...
5. `toDetails` assembles the **nested** response: `groupClassifiedItemsByShow` buckets each line under its show (own `showProduct.show_id`, else one hop up `parent_order_item_id`), splitting by `cart_item_type` into per-show `booths`/`add_ons`/`sponsorships` with `product_name`/`purchased_name`, each line's `id`/`is_default_included`/`parent:{id,type}` ref, and folded per-booth `setup_fee`/`cleaning_fee`; unresolvable-show lines land in the top-level **`unassigned`** bucket (logged). `buildFinancialSummary` sets conditional nulls; `payments[]` = settled only; `agreement` from the order's [OrderAgreement](../../relationship/2-entities/order-agreement.md), with `resolveSignatureUrl` turning its `signature_data` into a short-lived presigned `signature_url` (`null` for empty/bypass/non-URL values) via `UploadService`; `loadAdditionalPurchases` returns child add-on orders each with `show:{id}`.
...
- **`parent` joins within a show only (SBE-1147).** A bundled child's `parent.type` names which per-show array to search (`booth`/`add_on`/`sponsorship`, `workshop_pavilion`→`booth`) and `parent.id` matches a sibling line's `id` (the id-cipher is deterministic, so the two tokens are equal on the wire). Parent and child are always in the same show; a cross-show or unresolvable parent yields `parent: null`. Combine with `is_default_included` to fold/mark bundled lines instead of double-counting them.
- **`signature_url` is presigned and may be `null`.** It's a short-lived URL to the captured signature image, minted per-request via `UploadService.getAccessibleUrl`. It's `null` when there's no signature, for manual-bypass orders (`signature_data = 'MANUAL_SIGNED_AGREEMENT'`), or for any non-`http(s)` value — the client never receives a broken `<img>` src. The raw `signature_data`, `ip_address`, and `user_agent` are never exposed.
