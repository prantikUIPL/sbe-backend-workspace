...
- **Progressive disclosure (C4-style)** — start broad, zoom in on demand. The levels above are exactly that: glossary → story → per-entity → full map.
...
The editable Mermaid sources sit beside their renders (`2-entities/ego/*.mmd` for the cards, `diagrams/*.mmd` for the full maps). To re-render (needs Node + a Chrome/Chromium install):
...
# loop over 2-entities/ego/*.mmd to rebuild all of them
```
