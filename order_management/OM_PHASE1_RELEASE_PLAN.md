...
- **P1 (code, RULED net 2026-07-05):** `balance_due` shipped GROSS on 3 surfaces (24.7 payments `order-payments.service.ts:66`, 24.8 plan-GET `order-payment-plan.service.ts:211`, order list `orders.service.ts:184`) vs NET on 24.6 (`order-details.service.ts:244`). Confluence: 24.6 is the only story specifying a refund-aware balance; 24.7/24.8 silent; 24.1 mandates a "centralized billing service" balance → **24.6 authoritative → align all 3 to net** (`deriveNetPaid`). Bundles with P3+P4 on ONE admin fix branch. Also revisit 24.7 `payment_status` label (gross → net) for the same reason.
- **P2 (doc, DONE above):** 24.15 reminder recipient = `billing_email`-only is spec-compliant ("to customers"); recorded in P-D7 + ledger.
- **P3 (admin code, doc-only):** 24.10-c Swagger `order-cancel.dto.ts:218/:107` still says `processing→canceled`; code cancels scheduled+failed only (409-blocks processing). Amend both strings.
- **P4 (admin code, hygiene):** U11 seeder URL `permission.seeder.ts:1351/1359` camelCase `:installmentId` vs live snake `:installment_id`; align (auth keys off the permission key, cosmetic).
...
**Remaining:** admin fix branch for P1+P3+P4 (Prantik to authorize the branch cut) → gate → PR → merge; then the wider close-out (Implementation-Status doc updates per story, ORDER_EPICS_BUILD_CONSOLIDATION refresh, memory/plan → "implemented"). Accepted known-items unchanged: SCH-8 dedupe granularity, 24.10 "Voided" wording, U10 intra-release drift (rescued pre-merge).
