...
| P-D7 | 24.15 D1–D5 | Email-only; settings family 7/7/14 pending BA **with gate-provenance doc-comments (user directive)**; independent cron; self-seeded slugs; manual-rows-only predicates; **recipient = `billing_email` only** (satisfies the 24.15 "to customers" spec — no fallback chain mandated; deliberately simpler than 24.11's U6 `billing_email → company contact → skip` chain; audit P2 2026-07-05, doc-only) |
...
**Phase 6 UPDATE (2026-07-05): LIVE SMOKE GREEN + PRE-PUSH GATE GREEN — awaiting Prantik's commit authorization (admin only).** Cancelled-vs-void gap reviewed with Prantik before resuming: confirmed the relabel already fully resolves it (it satisfies 24.6-q/z and realigns the already-merged 24.7/24.8 displays); reuse of the `canceled` PT status by both manual-void (24.6-z) and the 24.10 cancel-cascade is correct-by-spec (the sheet has ONE `canceled` row = "Voided"); no deliverable lost; the one residual — 24.10 cascade rows now also render "Voided" — is captured as a Phase-8 §8.2 verify item. Restart cost nothing (relabel was on disk pre-restart). **LIVE SMOKE (booted `dist/src/main.js` :3333, real HTTP, cookie-JWT superadmin, encrypted-id routing; SMOKE-1130 fixtures EX/AD/SUB/CX/VD):** all 18 status-code cases + body/DB assertions PASS. GET EX → item tree booths→add_ons, coupon via cart, agreement, plan `1:Paid(succeeded)/2:Unpaid(scheduled)`, totals net_paid/balance_due, `source: Self-Service`, **NO `notes` key**; GET CX → `Cancelled (Partial Refund)` (paid 500/refund 200/net 300/balance 200); GET AD → null billing + fees 100+50→950; GET SUB → `Active (Paid In Full)`, `source: Subscription`, no cart; raw-int→400, unknown→404, unauth→401. PATCH billing+emails persisted (Boston, emails replaced wholesale), canceled→409, bad email→400. sales-rep Admin→200 (persisted sales_person_id=1 + audit), Developer(ineligible)→400. Void scheduled(pt5)+failed(pt4)→**milestone_status "Voided"**, DB `canceled`+`next_retry_at=NULL`+audit; succeeded(pt6)→409, cross-order→404, raw-int→400. **DB persistence verified** incl. `Order.notes` untouched by write paths (still the idempotency string, never returned). **PRE-PUSH GATE (admin 8-step) GREEN:** npm ci ✓, prisma generate ✓, DB reset DROP-FORCE+CREATE+migrate deploy (168 migrations, additional_emails lands) ✓, seed `run-seeds.ts` ✓ (4 order perms seeded on virgin DB; `seed:plans` skipped — needs a Stripe key absent on this machine, out-of-scope for 24.6), typecheck ✓, lint:check ✓, **test:cov 3335/3335 (220 suites) POST-RELABEL** ✓ (caveat above RESOLVED; no SIGSEGV flake this run), build ✓. **REMAINING:** commit/push (`feat(SBE-1130)`, admin only) [awaiting authorization] → PR (Prantik) → merge → rebase → mark Phase 6 COMPLETE + note the relabel corrects 24.7/24.8.

**Phase 6 STATUS (2026-07-05): COMPLETE — MERGED to dev + REBASED.** Admin `4c3308c` `feat(SBE-1130)` pushed → pipeline #876 FAILED at the SonarQube gate only (gitleaks ✓, lint/typecheck/test ✓): ONE new-code violation `typescript:S4624` (MAJOR, nested template literal) at `order-update.service.ts:193`, author `prantik@` (ours) — fixed by hoisting the fallback name to a `targetName` local (byte-identical audit note; eslint/tsc/spec 16/16 clean). Follow-up `5826f19` `fix(SBE-1130): resolve SonarQube S4624` → pipeline #877 SUCCESSFUL, **SonarQube gate OK (all conditions, new_violations=0)**. **MERGED via PR#537 → dev `64ef8e3`** (verified: our commit is an ancestor, S4624 fix present in dev). Rebased all 5 repos onto dev — **all 5 == dev** (admin `64ef8e3` fast-forward, other four unchanged: exhibitor `c0bdf97`, worker `57fa323`, external `c6d9dd1`, pulse `c4a9d4b`). **The MILESTONE_STATUS_LABELS relabel shipped in this phase now governs the already-merged 24.7 (Payments View) and 24.8 (Payment Plans) installment displays too — they render Unpaid/CC Failed/Voided from the single shared map (no separate change needed in those stories).** **STORY 24.6 COMPLETE.** Infra: admin + workspace repos both push via `core.sshCommand` → `pmtool_key_file` (same key authenticates GitHub + Bitbucket, restored post-restart). Only Phase 7 (24.15 Payment Reminders, SBE-1139) remains before the Phase 8 delivery audit.
...
**Phase 7 STATUS (2026-07-05): COMMITTED + PUSHED (worker + admin), awaiting Prantik's PRs.** Two-repo phase (worker cron + admin seeds; other 3 clean). Build (Opus workflow, 16 agents: 5 recon → 2 parallel builds → 3 review lenses → adversarial verify): WORKER independent `payment-reminder` cron pair CLONED from payment-charge (registrar+task in `src/scheduler/payment-reminder/`, service+module in `src/jobs/payment-reminder/`, run-response DTO, dev-only `POST /manual-trigger/payment-reminders/run`, scheduler wiring, mailer layout-map +2 slugs); disjoint predicate `payment_type != credit_card` (mirror of charge guard); UPCOMING (once, in-window) + OVERDUE (repeat per interval, notification_logs dedupe); stop-on-settled by re-selection (NO new state); recipient billing_email; worker-mailer dispatch; settings `payment_reminder_{upcoming_days,overdue_interval_days,hour_utc}`=7/7/14 as worker-side defaults w/ gate-provenance doc-comments (OQ-1). ADMIN: append 2 trigger slugs + 2 EMAIL templates (email-only, ownership rule, append-don't-reorder). **Reviews: 3 lenses (spec a-h, clone-correctness/invariants, seeder-discipline/provenance/security) + adversarial verify → 2 CONFIRMED: (1) MAJOR `{{payment_method}}` token unpopulated → FIXED (PAYMENT_METHOD_LABELS map: check→Check, bank_wire_ach→Bank Wire / ACH, paypal→PayPal + regression assertion); (2) MINOR dedupe is email-level not per-installment (notification_logs has no order/PT anchor) → ACCEPTED (Option A) + registered as SCH-8 in the SCHEDULING known-issues register (per register-separation; the E&S scheduler that absorbs this cron per OQ-4 owns the anchor-column fix).** **LIVE SMOKE GREEN (real Postgres via harness running the actual PaymentReminderService; SendGrid unconfigured → FAILED sends but body persisted first):** RUN1 scanned=3 (UP upcoming + OD/DEDUP overdue; CARD/PAID/VOID excluded → stop-on-settled + card-disjoint hold), DEDUP skipped (pre-seeded SENT); RUN2 after flipping OD→SENT: OD now deduped, UP retries (FAILED doesn't suppress); rendered bodies carry "Check"/"Bank Wire / ACH" with NO literal `{{payment_method}}`; card/paid/void = 0 reminder logs. **GATES GREEN: worker 6-step (npm ci, prisma generate, typecheck, lint [0 err; 1 pre-existing cart-maintenance warning not ours], test:cov 319/319, build) ×2 runs; admin 8-step w/ DB reset + FULL seed:run incl. seed:plans (Stripe TEST key borrowed from external-api .env → test products created), typecheck, lint, test:cov 3355/3355, build; virgin-DB check = 2 reminder slugs + 2 EMAIL templates.** **COMMITTED + PUSHED 2026-07-05: worker `d98fa5f`, admin `cc69bcf` → origin/feature/SBE-1125** (worker commitlint enforces body ≤100 chars — one line shortened). **REMAINING:** Prantik raises 2 PRs → pipelines/SonarQube → merge → rebase → Phase 7 COMPLETE. Then Phase 8 (Delivery Audit) is the only thing left in the release.

**Phase 7 STATUS (2026-07-05): COMPLETE — MERGED to dev + REBASED.** Worker `d98fa5f` (PR#28) + admin `cc69bcf` (PR#538) → pipelines #38 / #878 both SUCCESSFUL, **both SonarQube gates OK (no new-code issues on either — zero Sonar fixes needed)**. MERGED to dev (worker `34b234b`, admin `fad8726`; both our commits verified ancestors). Rebased all 5 repos → **all 5 == dev** (admin `fad8726`, worker `34b234b`, exhibitor `c60d72f` [dev moved via another team's merge — realigned], external `c6d9dd1`, pulse `c4a9d4b`). **STORY 24.15 COMPLETE. ALL 7 OM PHASE-1 BUILD STORIES SHIPPED (24.9/24.10/24.11/24.8/24.7/24.6/24.15).** Only **Phase 8 (Delivery Audit)** + release close-out (Post-Atom Checklist: Implementation-Status doc updates, consolidation-doc refresh, optional Jira transitions, memory/plan → implemented) remain.
...
**Phase 8 STATUS (2026-07-05): COMPLETE — release RECONCILED.** Read-only multi-agent audit (23 agents: 7 per-story traces → 7 adversarial verifies → 5 shared-unit conformance → 2 scope/verdict-flip → completeness critic → synthesis). Full report: **`OM_PHASE1_DELIVERY_AUDIT.md`**. **Result: 68 plan-time Deliverable+Partial across all 7 stories → 65 verified-delivered + 2 partial-as-planned (24.9-f Stripe-detect half, 24.8-l label display) + 1 doc-only drift (24.10-c) → 0 LOST, 0 unexplained drift.** All 20 shared units conform (incl. corrected U10 label map); `Order.notes` provably never exposed; no uncontrolled scope-creep or silent verdict-flip. Doc/Confluence/story cross-check confirmed every audit-traced count matches each story's doc-declared Deliverable+Partial (1st synthesis pass had dropped 24.6 via a pipeline-null on its failed verify stage; recovered + re-verified 22/22 — audit-integrity note in the report). **Confluence 3859742741 cross-checked** for the two substantive items. **Fix list (proposed; nothing committed):**
- **P1 (code, RULED net 2026-07-05) — ✅ SHIPPED:** `balance_due` shipped GROSS on 3 surfaces (24.7 payments `order-payments.service.ts:66`, 24.8 plan-GET `order-payment-plan.service.ts:211`, order list `orders.service.ts:184`) vs NET on 24.6 (`order-details.service.ts:244`). Confluence: 24.6 is the only story specifying a refund-aware balance; 24.7/24.8 silent; 24.1 mandates a "centralized billing service" balance → **24.6 authoritative → aligned all 3 to net** (`deriveNetPaid`) + 24.7 `payment_status` label (gross → net). **Fixed in `fix(SBE-1131)` `7fd7a9b`, admin PR#539 → dev `55c6e61` (gate green 3360/3360 + DB reset; 4-view consistency smoke PASS; pipeline #879 SUCCESSFUL).**
- **P2 (doc, DONE above):** 24.15 reminder recipient = `billing_email`-only is spec-compliant ("to customers"); recorded in P-D7 + ledger.
- **P3 (admin code, doc-only) — ✅ SHIPPED:** 24.10-c Swagger `order-cancel.dto.ts:218/:107` said `processing→canceled`; corrected to "scheduled | failed → canceled; a processing installment blocks with 409". Shipped in `7fd7a9b` (PR#539).
- **P4 (admin code, hygiene) — ✅ SHIPPED:** U11 seeder URL `permission.seeder.ts` camelCase `:installmentId` → snake `:installment_id` (auth keys off the permission key, cosmetic). Shipped in `7fd7a9b` (PR#539); virgin-DB seed verified snake_case, 0 camelCase leftovers.
- **P5 (doc, DONE above):** ledger U1 (+refund-options/`orders.refund.read`), U12 (+`create`), U3 (net-balance canonical) reconciled to the delivered surface.

**Remaining:** ✅ P1+P3+P4 fix SHIPPED (admin `7fd7a9b`, PR#539 → dev `55c6e61`; all 5 repos == dev). ✅ Per-story Implementation-Status doc updates DONE 2026-07-05 (7 OM stories + 24.1, `.md` + `.xlsx`; shared plan copied into each story subfolder). Still open (close-out, tracked separately): `ORDER_EPICS_BUILD_CONSOLIDATION.md` refresh; memory/plan → "implemented"; optional Jira transitions (on explicit ask only). Accepted known-items unchanged: SCH-8 dedupe granularity, 24.10 "Voided" wording, U10 intra-release drift (rescued pre-merge). **NOTE:** this does NOT close the release — a final cross-epic pass over Epic 24 (OM) + Epic 13 (Order History) requirements (esp. not-delivered items + open questions) is planned next, with a few more decisions + code changes before final closeout.
