...
| 10 | Custom listing "event selector" — story wording to be updated | Open — meaning clarified (2026-06-12): it references the dropdown the frontend needs to show for placeholders (select a trigger event → its placeholders are shown); BA to update the story wording accordingly | BA / Documentation | None — trigger-events `GET` already returns each event with `available_placeholders` |
...
| 19 | Predefined `channel_config` — **no API path back to the system-default `null`** once an editable key is written (found 2026-06-12, live HTTP smoke; **direction agreed 2026-06-12, implementation deferred**) | Open (direction agreed, not yet implemented) — all 20 seeded predefined rows ship with `channel_config = null` (the mailer's hard-coded behavior is the "system default"). Story 76.5 lets admins edit `from_name`/`reply_to`/`cc`/`bcc`, and the first such `PUT` materializes a `channel_config` object. **User clarification 2026-06-12: the additive merge is intentional and causes no runtime issue today** — nothing reads `channel_config` yet; the pending send wiring will consume keys **individually** (key present → use it, absent/empty → system default), so `cc`/`bcc` are already effectively clearable via `[]`. **Residual gap:** `from_name`/`reply_to` can never be unset once written — `null` is 400 by design (`IsOptionalNonNull`), `""` is 400 (`IsNotEmpty`, and `IsEmail` on `reply_to`), and the merge never removes keys — so they'd remain permanent overrides even under per-key wiring. **Agreed direction (2026-06-12): a clear-to-default sentinel on `PUT`** — a special value (proposed: the empty string `""`) on the clearable keys stores the key as `null` (matching the create path's unset representation); optionally normalize an all-cleared config back to `null`. **Implementation explicitly deferred by the user (2026-06-12) — do not code until directed** | Us (deferred; user to schedule) | None |
...
| 21 | **Live send path does not distinguish predefined from custom templates** (found 2026-06-12, release-note implementation audit) | Open — `MailerService.sendFromTemplate` (`src/common/services/mailer.service.ts` ~L114–121) selects the template with `findFirst({ notification_type, channel: 'EMAIL', is_active: true })` — **no `is_predefined` filter and no `orderBy`** — so an active **custom** template created on a dispatched (live) trigger slug is eligible for selection and can **nondeterministically shadow** the predefined template for that trigger. Live dispatched slugs today: `welcome_email`, `forgot_password`, `exhibitor_welcome_admin_created`, `cart_updated_notification` (admin backend); `welcome_email_exhibitor`, `exhibitor_forgot_password`, `company_user_invitation`, `invitation_accepted_to_exhibitor`, `contact_us_acknowledgment`, `contact_us_admin_notification`, `ppl_subscription_canceled` (exhibitor backend); `ppl_order_confirmation`, `ppl_subscription_renewal`, `ppl_product_order_payment` (external API); `lead_assigned_preview`, `lead_daily_summary`, `low_balance_warning` (background worker). Custom templates on non-dispatched triggers are never sent (no general dispatcher exists). This undercuts the premise of #16, which assumed the send-time lookup is scoped to predefined rows. **Proposed fix (ours, not yet implemented):** add `is_predefined: true` (plus a deterministic `orderBy`) to the template-lookup in every repo that sends from templates — **planned to be implemented together with the scheduling logic** (per user, 2026-06-12); until it ships, avoid active custom templates on live triggers in shared environments | Us (code fix) | None |
| 22 | **`lead_assigned_preview` seeded copy conflicts with its own trigger's placeholder list** (found 2026-06-12, release-note implementation audit) | Open — the seeded subject/body use `{{attendeeName}}`, `{{attendeeEmail}}`, `{{attendeeMobile}}`, `{{attendeeZip}}`, `{{myLeadsUrl}}`, but the trigger's `available_placeholders` are `expiryHours`, `attendeeFirstName`, `attendeeZipPrefix`, `categoryList`, `claimUrl` — zero overlap. Consequence: any `PUT` that touches `subject` or `body` re-validates placeholders against the trigger and is rejected with 400 (edits to other fields — name, status, config keys — still work). **Fix is a data correction on whichever side is wrong** (align the trigger's placeholder list with the copy the background-worker call site actually populates, or rewrite the copy to the trigger's list); since the template seeder is create-only (#15), the fix ships to seeded environments as a one-off migration or manual admin edit, not a seeder re-run | Us (data correction; check worker call site for which side is authoritative) | None |
| 23 | **`ppl_product_order_payment` ("Order Payment Receipt") body uses Handlebars block helpers the senders cannot process** (found 2026-06-12, release-note implementation audit; **code-verified**) | Open — the seeded body contains `{{#if is_fully_paid}}` / `{{else}}` / `{{#if next_installment_amount}}` blocks, but every mailer renders template bodies via a literal `split(token).join(value)` splice — verified in `external-api-service` (the sender of this slug): `src/common/services/mailer.service.ts` L111–119 splice the body; `Handlebars.compile` is used ONLY for the branded layout shell (`getLayout`, L188–196). Block markers are not in the `replacements` map, so they survive into the delivered email as literal text. Placeholder validation deliberately skips block tokens (`extractSimplePlaceholders`), so editing the template will not surface the problem either. **Fix options (decision needed):** flatten the body to simple tokens (e.g. pre-compute an `installment_note` token at the call site), or upgrade body rendering to real Handlebars (bigger change — the per-token HTML-escaping strategy would need rework). Recorded factually; the body shipped with the payment-receipt flow's seed data | Us / design decision | None |
...
## 10. Custom listing "event selector" — story wording to be updated
...
- **Ambiguity (original):** trigger-event selection in our design happens during **Create / Edit**, not on the listing. An event selector on the listing reads as either a filter or a create entry point.
- **Clarified (2026-06-12, per user):** the "event selector" references a **dropdown the frontend needs to show for placeholders** — selecting a trigger event surfaces its available placeholders. It is not a listing-page filter or create shortcut.
- **Action:** BA to update the story to describe the placeholder dropdown. No backend gap — the trigger-events `GET` endpoint already returns every event with its `available_placeholders`, which is exactly what such a dropdown consumes.
