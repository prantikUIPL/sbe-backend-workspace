...
# Pre-push stability gate for pulse-broker-service (5 steps)
...
if [[ "$PKG_NAME" != "pulse-broker-service" ]]; then
    echo "ERROR: Must run from pulse-broker-service root (found package: '$PKG_NAME')." >&2
    exit 1
fi
