...
# Pre-push stability gate for external-api-service (5 steps)
...
if [[ "$PKG_NAME" != "sbe-external-api-service" ]]; then
    echo "ERROR: Must run from external-api-service root (found package: '$PKG_NAME')." >&2
    exit 1
fi
